...
TEST(Box2dTest, RelationToPoint) {
  const auto compute_distance = [](const Box2d& box, const Vec2d& point) {
    ...
    double double_area =
        std::abs(CrossProd(corners[0] - point, corners[1] - point)) +
        std::abs(CrossProd(corners[1] - point, corners[2] - point)) +
        std::abs(CrossProd(corners[2] - point, corners[3] - point)) +
        std::abs(CrossProd(corners[3] - point, corners[0] - point));
